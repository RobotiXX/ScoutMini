...
BAG_ROOT="${BAG_ROOT:-${XDG_STATE_HOME:-$HOME/.local/state}/scoutmini/rosbags}"
DURATION_SECONDS="${DURATION_SECONDS:-60}"
STAMP="$(date +%Y%m%d_%H%M%S)"
OUT_DIR="${OUT_DIR:-$BAG_ROOT/zed_debug_$STAMP}"

TOPICS=(
  "${ZED_RGB_TOPIC:-/zed2/zed_node/rgb/color/rect/image/compressed}"
  "${ZED_DEPTH_TOPIC:-/zed2/zed_node/depth/depth_registered}"
  "/tf"
  "/tf_static"
)
