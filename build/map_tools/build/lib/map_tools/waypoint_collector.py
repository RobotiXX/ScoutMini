...
class WaypointCollector(Node):
    def __init__(self) -> None:
        ...
        self.selected_waypoint_pub = self.create_publisher(
            String,
             '/selected_waypoint',
             10
         )
    ...
    def _interactive_feedback_cb(self, feedback: InteractiveMarkerFeedback) -> None:
        ...
        if feedback.event_type == InteractiveMarkerFeedback.BUTTON_CLICK:
            ...
            if feedback.control_name == 'edit':
                ...
                selected_name = self.waypoints[index]['name']

                msg = String()
                msg.data = selected_name

                self.selected_waypoint_pub.publish(msg)

                self.get_logger().info(
                    f"Selected waypoint: {selected_name}"
                )
            return
